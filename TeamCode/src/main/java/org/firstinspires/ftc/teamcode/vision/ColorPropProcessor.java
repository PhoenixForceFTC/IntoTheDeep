...
public class ColorPropProcessor implements VisionProcessor {
    ...
    public Object processFrame(Mat frame, long captureTimeNanos) {
        ...
        leftAvg = Core.sumElems(grayscale.submat(new Rect(0, 0, width / 2, grayscale.height())))
                .val[0] / (width * height);
        rightAvg = Core.sumElems(grayscale.submat(new Rect(width / 2, 0, width / 2, grayscale.height())))
                .val[0] / (width * height);
    ...
    public double getRightAvg(){return rightAvg;}
    public double getLeftAvg(){return leftAvg;}
}
